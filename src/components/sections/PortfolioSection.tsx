...
export default function PortfolioSection({ onOpenQuote }: PortfolioSectionProps) {
  ...
  return (
    <section ref={sectionRef} id="portfolio" className="py-20 md:py-28 bg-charcoal-navy text-warm-offwhite relative border-t border-navy-border/60">
          ...
          <div className="flex items-center gap-2 bg-navy-card/80 p-1.5 rounded-2xl border border-navy-border self-start md:self-auto">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all ${
                viewMode === '3d'
                  ? 'bg-vivid-teal text-charcoal-navy shadow-teal-glow'
                  : 'text-warm-offwhite/70 hover:text-vivid-teal'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${
                selectedCategory === cat.id
                  ? 'bg-vivid-teal border-vivid-teal text-charcoal-navy shadow-teal-glow'
                  : 'bg-navy-card/50 border-navy-border hover:border-vivid-teal text-warm-offwhite'
              }`}
            >
              {cat.label}
            </button>
        ...
        {viewMode === '3d' ? (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-navy-card/60 border border-navy-border rounded-2xl">
              <button
                onClick={handlePrev}
                data-cursor="magnetic"
                className="px-4 py-2 bg-charcoal-navy border border-navy-border hover:border-vivid-teal rounded-xl text-xs font-bold text-warm-offwhite flex items-center gap-1 transition-all shadow-sm"
              >
                <ChevronLeft className="w-4 h-4 text-vivid-teal" />
              ...
              >
                ...
                <ChevronRight className="w-4 h-4 text-vivid-teal" />
              </button>
            </div>
          </div>
        ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map((item, idx) => (
              ...
              >
                <div className="relative w-full aspect-[4/5] overflow-hidden bg-dark-bg">
                  <Image
                    src={item.image}
                    alt={item.title}
                    fill
                  ...
                  <div className="absolute inset-0 bg-dark-bg/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <span className="px-4 py-2 bg-vivid-teal text-charcoal-navy font-bold text-xs rounded-full shadow-lg flex items-center gap-1.5">
                      <Eye className="w-3.5 h-3.5 text-charcoal-navy" />
                ...
                <div className="p-4 bg-navy-card border-t border-navy-border/60">
                  <div className="flex items-center gap-1.5 text-[10px] font-bold text-vivid-teal uppercase tracking-wider mb-1">
                    <Tag className="w-3 h-3" />
                    <span>{item.categoryLabel}</span>
                  </div>
                  <h3 className="font-display font-bold text-base text-warm-offwhite group-hover:text-vivid-teal transition-colors">
                    {item.title}
                  </h3>
